Function Get-AllTenantAdvancedThreatProtectionTrafficReport {
    [CmdletBinding()]
    param (
        [parameter(
            Mandatory = $false,
    ...
    . "$here\..\Get-TenantAdvancedThreatProtectionTrafficReport\Get-TenantAdvancedThreatProtectionTrafficReport.ps1"
    ...
    foreach ($tenant in $tenants) {
        $result = $null
        $result = Get-TenantAdvancedThreatProtectionTrafficReport -DelegatedAdminCred $DelegatedAdminCred -TenantDomainName $tenant.DefaultDomainName
        if($null -ne $result)
        {
