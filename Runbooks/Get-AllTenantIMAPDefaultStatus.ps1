...
$job = Start-AzureRmAutomationRunbook -Name "Get-TenantIMAPDefaultStatus" -AutomationAccountName "AzureAutomation" -ResourceGroupName "PowerBI" -Parameters $params
...
$data = New-Object PSObject -Property @{
    Tenant           = $tenant.Name
    IMAPandPOPAllowed = $jobResults.value[1]
...
WHERE TABLE_NAME = 'ExchangeOnlineIMAPDefaultStatus')
CREATE TABLE [dbo].[ExchangeOnlineIMAPDefaultStatus] (
[Tenant] varchar(100),
[IMAPandPOPAllowed] bit,
[Date] datetime
)"
...
foreach($item in $mergedObject)
{
    ...
    $IMAPandPOPAllowed = $item.IMAPandPOPAllowed
    $Date = $item.date
    $params.Query = "
